...
class Automobil(Vehicul):
    def __init__(self, motor: Motor, culoare: str, firma: str, vopsea_metalizata: bool, aer_conditionat: bool,
        ...
        self.__firma = firma
        self.__vopsea_metalizata = vopsea_metalizata
    ...
    def print_caracteristici(self):
        print(f"Caracteristici:\n"
              f" Motorul are {str(self.__motor.get_nr_cilindri())} cilindrii, "
              f"putere maxima este {str(self.__motor.get_max_putere())}"
              f"si are catalizatorul {self.__motor.get_catalizator()}.\n "
              f"   Masina are {str(self.get_nr_roti())} roti, capacitatea rezervolui este de {str(self.get_capacitate_rezervor())},"
              f"si are consumul urban de {str(self.get_consum_urban())}."
              f"   Culoarea este {self.get_culoare()}, {'Are' if self.get_vopsea_metalizata() else 'Nu are'} vopsea metalizata")
